...
static int fdt_uart_module_init(const void *fdt, int uart_offset, const char *node[], int node_max)
{
    ...
    for (i = 0; i < node_max; i++) {
        ...
        /* 大帧上传保护: WB2-clock 图片块帧 ≤2021 字符, 512B RX 流缓冲装不下,
         * yloop 被 LVGL/wifi 抢占 >2.5ms(2M 波特)就丢字节 -> 帧断、无 ACK、块超时。
         * 放大到 4KB 让整帧进缓冲, 接收不依赖并发排空 */
        if (rx_buf_size < 4096) rx_buf_size = 4096;
