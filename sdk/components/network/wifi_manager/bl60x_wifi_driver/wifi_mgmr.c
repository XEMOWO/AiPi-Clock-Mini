...
static void stateConnectedIPNoEnter(void *stateData, struct event *event )
{
    ...
    /* [WB2-clock fork] AP-recover 机制禁用:
     * 原逻辑在 STA 连接信道 != AP 信道时, 3s 后停掉 AP 让位 STA, 2s 后再把 AP 拉起来。
     * 实测在 BL602 单射频上, 停 AP 的瞬间会把 STA 链路一起打掉(收到 deauth),
     * 且 STA 进入 disconnect 态后自动重连被禁用、接口被移除 → STA 再也回不来,
     * 设备卡死在配网 AP 模式。本项目配网流程自行管理 AP 生命周期
     * (GOT_IP 后 8s 窗口给手机轮询, 然后 POR 重启), 不需要此机制。 */
#if 0
    if (wifiMgmr.inf_ap_enabled) {
        int channel;
