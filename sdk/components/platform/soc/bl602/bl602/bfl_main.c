...
/* 应用极早期钩子: 串口与 flash 已就绪 (bl_sys_early_init 已调 bl_flash_init),
 * 调度器/easyflash 尚未启动。WB2-clock2 用它做"上电第一时间开机计数"
 * (连续按 EN 5 次进配网)。弱符号: 其他工程不定义则为空操作。 */
void __attribute__((weak)) app_boot_early(void) {}
...
void bfl_main()
{
    ...
    app_boot_early();   /* 上电第一时间计数: 串口就绪后、"Booting" 横幅前 */
