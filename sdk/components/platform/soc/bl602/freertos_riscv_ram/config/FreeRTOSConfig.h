...
/* 24KB -> 40KB: newlib malloc/calloc 全部走 pvPortMalloc (syscalls.c), 系统唯一堆。
 * 占用: 任务栈 ~10.4K (sntp 4K+wx 4K+mon 4K+wifi 3K+idle/timer) + mbedtls TLS 会话峰值 ~21K
 * (in/out 8K*2 缓冲 + 上下文 + 握手临时)。24576 时 mbedtls_ssl_setup 分配 out_buf 失败
 * (剩 14192 < 8193), 导致和风 HTTPS 全部失败。 */
